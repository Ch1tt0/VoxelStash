...
SDL_AppResult SDL_AppInit( void** appstate, int argc, char** argv )
{
	...
	app->window =
		 SDL_CreateWindow( "VoxelStash", 1280, 720,
								 SDL_WINDOW_RESIZABLE | SDL_WINDOW_HIGH_PIXEL_DENSITY );
	if ( !app->window )
		return sdl_error();

	app->gpu_device = SDL_CreateGPUDevice( SDL_GPU_SHADERFORMAT_SPIRV |
...
SDL_AppResult SDL_AppIterate( void* appstate )
{
	...
	if ( !SDL_WaitAndAcquireGPUSwapchainTexture(
				command_buffer, app->window, &swapchain_texture, &swapchain_width,
				&swapchain_height ) )
		return sdl_error();
